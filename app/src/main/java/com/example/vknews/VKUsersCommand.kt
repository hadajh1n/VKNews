...
class VKUsersCommand(private val uids: IntArray = intArrayOf()) : ApiCommand<List<VKUser>>() {
    override fun onExecute(manager: VKApiManager): List<VKUser> {
        if (uids.isEmpty()) {
            // Если uids пустой, возвращаем данные текущего пользователя
            ...
            // Если переданы uids, получаем данные указанных пользователей
    ...
    companion object {
        const val CHUNK_LIMIT = 900 // Лимит для разбиения запросов
    }
